...
Only a fixed allowlist is permitted: `discover`, `snapshot`, `planApply`, `apply`, `save`, `rollback`, and the no-payload `undo` operation. Every request and response carries protocol version, nonce/request ID, operation, and schema-valid structured-cloneable payload. The isolated side accepts replies only from its injected bridge identity and matching request ID. `undo` never accepts a caller-supplied rollback token; the MAIN-world owner keeps that state private. No `eval`, arbitrary property path, function name, unrestricted fetch, or extension API enters MAIN world. Invalid schema, duplicate request, origin/source mismatch, timeout, and unsupported operation fail closed without mutation. The bridge must serialize one apply transaction per editor tab.

## Contract C4 — Live transaction, save, and rollback

`snapshot` captures type/id-or-unsaved, live metadata, tags in display order, Lexical state/emptiness, dirty state, `updated_at` if available, and save activity. `planApply` resolves every dependency and validates all modes before changing state. A read-only preview renders the resulting field actions without mutation. `apply` mutates the live record/editor only after a valid plan; then invokes exactly one Ghost-native save transaction and verifies returned clean state, current `updated_at`, and applied live fields when the adapter supports readback. If mutation or save fails, attempt rollback from the pre-apply snapshot through the same live path; if rollback cannot be proven, retain a recoverable failure record and block further apply. A successful transaction retains one opaque pre-apply snapshot in MAIN for a guarded one-use Undo; Undo refuses a changed resource/editor, restores and saves once, verifies the restored fields, and cancels the pending automatic refresh.
